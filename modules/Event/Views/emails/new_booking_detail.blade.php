    ...
    <table class="b-table" cellspacing="0" cellpadding="0">
        ...
        <tr>
            ...
            <td class="val">
                ...
                $activty = DB::table('bravo_events')->where('id',$booking->object_id)->first();
        ...
        <tr>
            <td class="label">{{__('Activity packages')}}</td>
            <td class="val">
                
               <?php
                
                $activtyname = DB::table('activity_packages')->where('id',$booking->package_id)->get();
                
                ?>
                
                  @foreach($activtyname as $aa)
                 <a>{{$aa->title}} </a> 
                 @endforeach
            </td>
       ...
       <tr>
            <td class="label">{{__('Travellers')}}</td>
            <td class="val">
               
              <?php
$traveller = json_decode($booking->travellers);
...
<a> {{ $sum }}</a>
        ...
        @endif
        <tr>
            <td class="label"><h3>{{__('Pricing')}}</h3></td>
        ...
        <tr>
            ...
            <td class="val fsz21"><strong style="color: #FA5636">{{$booking->currency}} {{intval($booking->total)}}</strong></td>
        </tr>
        <tr>
            <td class="label fsz21">{{__('Paid')}}</td>
            <td class="val fsz21"><strong style="color: #FA5636">{{$booking->currency}} {{intval($booking->paid)}}</strong></td>
        </tr>
        @if($booking->total > $booking->paid)
            <tr>
                <td class="label fsz21">{{__('Remain')}}</td>
                <td class="val fsz21"><strong style="color: #FA5636">{{$booking->currency}} {{intval ($booking->total - $booking->paid)}}</strong></td>
            </tr>
        @endif 
    </table>
</div>
