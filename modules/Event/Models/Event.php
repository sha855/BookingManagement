...
class Event extends Bookable
{
    ...
    public function getDetailUrl($include_param = true)
    {
        ...
        $urlDetail = app_get_locale(false, false, '/') . env('EVENT_ROUTER_PREFIX','activity') . "/" . $this->slug;
        if(!empty($param)){
    ...
    public static function getLinkForPageSearch( $locale = false , $param = [] ){
        ...
        return url(app_get_locale(false , false , '/'). env('EVENT_ROUTER_PREFIX','activity')."?".http_build_query($param));
    }
...
$dataTotalReview = $this->reviewClass::selectRaw("AVG(rate_number) as score_total, COUNT(id) as total_review")
    ->where('object_id', $this->id)
    ->where('object_model', "event")
    ->where("status", "approved")
    ->first();

if (!empty($dataTotalReview)) {
    
    if (!empty($dataTotalReview->score_total)) {
        $list_score['score_total'] = number_format($dataTotalReview->score_total, 0);
...
$list_data_rate = $this->reviewClass::selectRaw('SUM(CASE WHEN rate_number LIKE "5%" THEN 1 ELSE 0 END) AS rate_5,
                                                SUM(CASE WHEN rate_number LIKE "4%" THEN 1 ELSE 0 END) AS rate_4,
                                                SUM(CASE WHEN rate_number LIKE "3%" THEN 1 ELSE 0 END) AS rate_3,
                                                SUM(CASE WHEN rate_number LIKE "2%" THEN 1 ELSE 0 END) AS rate_2,
                                                SUM(CASE WHEN rate_number LIKE "1%" THEN 1 ELSE 0 END) AS rate_1')
    ->where('object_id', $this->id)
    ->where('object_model', "event")
    ->where("status", "approved")
    ->first()
