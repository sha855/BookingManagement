    ...
    .linkcolor-active{
        ...
        color: #dc3545 !important;
                ...
                <li class="{{$menuItem['class'] ?? ''}}" position="{{$menuItem['position'] ?? ''}}">
                  <a class="linkcolor" href="{{ url($menuItem['url'] ?? '') }}">
    @if(!empty($menuItem['icon']))
        <span class="icon text-center" ><i class="{{$menuItem['icon']}}  linkcolor"></i></span>
    @endif
...
<script>
    $(document).ready(function() {
        ...
        // Find all <a> elements with class "linkcolor"
        $("a.linkcolor").each(function() {
            // Check if the href attribute matches the current URL
            if ($(this).attr("href") === currentUrl) {
                // Add the "linkcolor" class to the matching link
                $(this).addClass("linkcolor-active");
            }
        });
    });
</script>
