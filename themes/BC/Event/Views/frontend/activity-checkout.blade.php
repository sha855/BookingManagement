...
$totalSavings = array_sum($discountSum);
          ...
          @endif
           ...
           <input type = "hidden"  name="object_id"   value=" {{$activityNAme->id}}">
           <input type = "hidden"  name="package_id"  value="{{$packageIdsString}}">
           <input type = "hidden"  name="object_name" value=" {{$activityNAme->title}}">
...
<div class="row">
  ...
  <div class="col-md-8">
    ...
    <h6 class="pt-4">  @if(Session('product'))
          <?php
    ...
    <p>
        ...
        @if(Session('formattedTotalPrice') && Session('product'))
...
?>
             ...
             <span style="color:#FF3500" id="realupcomingPrice" class="price pricex" data-price="{{$formattedPrice}}">{{$formattedPrice}}</span><span>
...
?>
              ...
              <span style="color:#FF3500" id="realupcomingPrice" class="price pricex" data-price="{{number_format($price,0)}}">{{number_format($price,0)}}</span><span>
...
?>
 ...
 $travellerdata = json_encode($data);
    ...
    <h6>Package</h6>
       ...
       @foreach($data['persons'] as $index => $pp)
  
            @if (!($data['quantityTotal'][$index] == "0"))
                <p>{{ $pp }}</p>  
            @endif
        @endforeach
  ...
  @foreach($quatitytoatl as $index => $tt)
    @if ($tt == 0 || $price[$index] == 0)
        @continue
    @endif
    <p style="width: 105%;">
        Quantity: {{ $tt }} |
        ...
        {{ number_format($price[$index],0) }} |
        
        
        
        
        Total Price: {{ number_format($tt * $price[$index],0) }}
    </p>
@endforeach
    ...
    <div class="row">
        ...
        <div class="col-md-6">
            <label>Apply Promocode </label>
             <img src="{{asset(url('images/Group.png'))}}" alt="Image" style="width: 19px;
    height: 19px;
    margin-right: 0px;
    top: 36px;
    left: -120px;
...
?>
              ...
              {{number_format($formattedPrice - $percent5Value,0)}}
...
?>
 ...
 <input type="hidden" value="{{$totalSavings}}" class="totalsavings" name="total_saving">
<input type ="hidden" class ="currencyName"value="{{$targetCurrency}}">
             ...
             <span id="totalsaving">{{$xformattedPrice}}</span>
...
<input type ="hidden" class ="currencyName"  value="AED">
              ...
              @endif
               ...
               <span id="totalsaving">{{$totalSavings}}</span> 
...
?>
        ...
        <input hidden="hidden" id="bookingPrice" value="{{$formattedPrice}}">
              ...
              @endif
              <span class="updated-price">{{$formattedPrice}}</span>  
...
?>
             ...
             <input  type="hidden"  id="bookingPrice" value="{{number_format($price,0)}}">
          ...
          <?php
           ...
           ?>
             ...
             @if($targetCurrency == "USD")
              ...
              <input type="hidden" value="{{$formattedPrice}}" class="priceshow" name="price">
  
             @else
             
                <input type="hidden" value="{{number_format($price,0)}}" class="priceshow" name="price">
