...
<div class="bravo_filter mx-3">
    <form action="{{url(app_get_locale(false,false,'/').env('EVENT_ROUTE_PREFIX','activity'))}}" class="bravo_form_filter">
        ...
        <ul>
            ...
            @foreach ($locations as $index => $location)
                <input type="checkbox"
                       ...
                       name="location_id"
                       value="{{ $location->id }}"
                       {{ $location->id == $lastLocationId ? 'checked' : '' }}>
...
<script>
    ...
    checkboxes.forEach(checkbox => {
       ...
       @php
        ...
        @foreach ($attributes->reverse() as $item)
