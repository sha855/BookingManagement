...
<div class="item-loop {{$wrap_class ?? ''}} h-70">
...
<div class="service-wishlist {{$row->isWishList()}}"  data-id="{{$row->id}}" data-type="{{$row->type}}">
    ...
    <div class="item-title mt-2 pt-2">
       <h6 > <a @if(!empty($blank)) target="_blank" @endif href="{{$row->getDetailUrl($include_param ?? true)}}">
            @if($row->is_instant)
    ...
    @endif
              ...
              &nbsp; &nbsp;&nbsp;<span class="rate-text" style="color:black; margin-left:-10px;">{{$reviewData['review_text']}}</span>
            </span>
            {{-- <span class="review">
...
?>
 ...
 <p style="position: relative;
  ...
  <span style="font-size: 25px; color: black; font-weight: 700;">{{ $discountformattedPrice }}</span>
 ...
 <div class="btn btn-light Daily-btn" style="position: relative;top:-3px; font-size:10px; border-radius:4px;">{{ number_format($dddiscountPercentage,0) }}% off</div>
  @endforeach
...
<p style="position: relative; top: 9px; margin-top: -18px;">
  <span style="font-size: 25px; color: black; font-weight: 700;">{{ $ff->discount }}</span>
  <span style="font-size: 24px;">{{ strtoupper($mainCurrency->val) }}</span>
</p>

<span style="font-size: 15px; color: black; font-weight: 700; text-decoration: line-through;">{{ $row->price }}</span>
...
<div class="btn btn-light Daily-btn" style="position: relative;top:-3px;font-size:10px; border-radius:4px;">
  @if($dddiscountPercentage)
    {{ number_format($dddiscountPercentage,0) }}% off
  @endif
</div>
@endforeach
