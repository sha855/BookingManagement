...
<div class="bravo_single_book_wrap @if(setting_item('event_enable_inbox')) has-vendor-box @endif">
    ...
    ?>
             <form id ="xpackagesForm" method="post" action="{{url('Quick-Checkout')}}" class="ssDIV">
                        @csrf
...
<h3 style="
    padding-left: 60px;
    margin-top: -26px;" class="text-black"><span style="color: rgb(255, 53, 0) !important;">{{ str_replace(',', '', number_format($formattedPrice, 0)) }}</span> <small style="margin-left: 41%;
    margin-top: -11%; margin-left: -10px;">  &nbsp;{{ $targetCurrency }}</small></h3>
...
<input type ="text" style="display:none;" id = "xparentnameofpackages" name ="price[]" value="{{ str_replace(',', '', number_format($formattedPrice, 0))}}"> 
...
?>
<div class="row">
<input type ="hidden"  id = "xparentnameofpackages" name ="price[]" value=" {{ str_replace(',', '', number_format($rooms->price - $rooms->discount_price, 0)) }}"> 

<div style="white-space: nowrap;">
    <span class="text-item2">{{ str_replace(',', '', number_format($rooms->price, 0)) }} AED</span>
    <h3 class="text-black" style="display: inline; color: rgb(255, 53, 0) !important; padding-left: 0px; margin-top: -26%; margin-right: 10px;"> {{ str_replace(',', '', number_format($rooms->price - $rooms->discount_price, 0)) }}</h3>
    <small style="display: inline; margin-top: -11%; margin-left: -10px;">{{ strtoupper($mainCurrency->val) }}</small>
</div>
</div>
<?php
}
?>        </p>
    ...
    ?>
        ...
        @endforeach
          ...
          <button type="submit" href="#" data-room-id="{{ $rooms->id }}"  class="btn btn-light w-100 mb-3" style="border-radius: 10px; border: 1px solid #FFF5E9; background: var(--light, #FFF5E9);">Quick Checkout</button>
         
          <button type="button" href="#" data-room-id="{{ $hotel->id }}"     class="btn btn-light w-100 card-btn mb-3 cartSubmitButton">ADD TO CART</button>
