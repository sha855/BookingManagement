  ...
  @if(count($staycation) > 0)
    @foreach($staycation as $wishlist)
        <div class="col-md-4 mb-4">
             ...
             <span id="heartIcon" class="fa fa-heart fa-3x fass newhotelheartstatus{{$wishlist['hotel']->id}} hotelwishlistaddingheart" style=" 
    right: 10px;
    z-index:2;
    padding: 6px 6px;
    border-radius: 30px;
    ...
    width: 30px;
    color: red;
"  attr="{{$wishlist['hotel']->id}}"></span> 

             <a href="{{ url('/staycation/' . $wishlist['hotel']->slug) }}" style="text-decoration:none;">
            <div class="card" style="border-radius: 10px;">
                
                <div class="Daily-Deals1" style="position: relative;">
                    <img src="{{ $wishlist['file_path']}}" style="height: 240px; width:100%; border-radius: 10px;">
                   

 <input type ="text" class="objectidgetclass{{$wishlist['hotel']->id}}" style="display:none;" name="object_id" value="{{$wishlist['hotel']->id}}">
  <input type ="text" class="objectmodalgetclass{{$wishlist['hotel']->id}}" style="display:none;" name="object_model" value="hotel">
                ...
                <div class="card-body" >
                    ...
                    <p>
                        <span class="btn btn-light Daily-btn">{{  $wishlist['hotel']->review_score}} <i class="fa fa-star"></i></span> 
                        </span>
                    </p>
...
?>
    <span style="font-size: 25px; color: black; font-weight: 700;">{{ intval($wishlist['hotel']->price) }}</span>
    <span style="font-size: 25px;">{{ $targetCurrency }}</span>
<?php
} else {
...
<span style="font-size: 25px; font-weight: 700; color: black;">
    {{ intval($wishlist['hotel']->price - ($wishlist['hotel']->price * ($wishlist['hotel']->discount_percent / 100))) }}
</span>
<span style="font-size: 25px;">
    {{ strtoupper($mainCurrency->val) }}
</span>
</p>
        
                    <p style="position: relative;
                      top: 18px;">
                        <span style="font-size: 15px; color: black; font-weight: 700; text-decoration: line-through;">{{ intval($wishlist['hotel']->price) }}</span>
                        <span style="font-size: 15px; color: black; font-weight: 700; text-decoration: line-through;">  {{ strtoupper($mainCurrency->val) }} </span>
                        <span class="btn btn-light Daily-btn">{{  $wishlist['hotel']->discount_percent}}% OFF</span>
                    </p>
                </div>
...
@endif
 ...
 @if(count($activitiesall) > 0)
    ...
    height: 30px;
    width: 30px;
    color: red;"  attr="{{$wishlist['hotel']->id}}"></span> 
    
    
             <a href="{{ url('/activity/' . $wishlist['hotel']->slug) }}" style="text-decoration:none;">
            <div class="card" style="border-radius: 10px;">
  ...
  <input type ="text" class="objectidgetclass{{$wishlist['hotel']->id}}" style="display:none;" name="object_id" value="{{$wishlist['hotel']->id}}">
  <input type ="text" class="objectmodalgetclass{{$wishlist['hotel']->id}}" style="display:none;" name="object_model" value="event">
                </div>
...
<span style="font-size: 25px; font-weight: 700; color: black;">
    {{ intval($wishlist['hotel']->price - ($wishlist['hotel']->price * ($wishlist['hotel']->discount / 100))) }}
</span>
<span style="font-size: 25px;">
    {{ strtoupper($mainCurrency->val) }}
</span>
                    ...
                    <p style="position: relative;
                       top: 18px;">
                        <span style="font-size: 15px; color: black; font-weight: 700; text-decoration: line-through;">{{ intval($wishlist['hotel']->price) }}</span>
                        <span style="font-size: 15px; color: black; font-weight: 700; text-decoration: line-through;">  {{ strtoupper($mainCurrency->val) }} </span>
                        <span class="btn btn-light Daily-btn">{{  $wishlist['hotel']->discount}}% OFF</span>
                    </p>
                </div>
